...
__all__ = ['MNASNet', 'mnasnet0_5', 'mnasnet0_75', 'mnasnet1_0', "mnasnet1_0_modified",
           'mnasnet1_3']
...
class MNASNet_modified(torch.nn.Module):
    """ MNASNet, as described in https://arxiv.org/pdf/1807.11626.pdf. This
    implements the B1 variant of the model.
    >>> model = MNASNet(1000, 1.0)
    >>> x = torch.rand(1, 3, 224, 224)
    ...
    def __init__(self, alpha, num_classes=2, dropout=0.2):
        ...
        layers = [
            ...
            _stack(depths[6], depths[7], 3, 1, 6, 1, _BN_MOMENTUM),
            # Final mapping to classifier input.
            nn.Conv2d(depths[7], 2048, 1, padding=0, stride=1, bias=False),
            nn.BatchNorm2d(2048, momentum=_BN_MOMENTUM),
            nn.ReLU(inplace=True),
        ]
        self.layers = nn.Sequential(*layers)
        self.classifier = nn.Sequential(nn.Dropout(p=dropout, inplace=True),
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict,
        ...
        super(MNASNet_modified, self)._load_from_state_dict(
            state_dict, prefix, local_metadata, strict, missing_keys,
            unexpected_keys, error_msgs)
...
def _load_pretrained(model_name, model, progress):
    ...
    model.load_state_dict(
        model_zoo.load_url(checkpoint_url))
...
def mnasnet1_0_modified(pretrained=False, progress=True, **kwargs):
    skip_list = {'layers.14.weight', 'layers.15.weight', 'layers.15.bias',
                 'layers.15.running_mean', 'layers.15.running_var',
                 'layers.15.num_batches_tracked', 'classifier.1.weight',
                 'classifier.1.bias'}
    model = MNASNet_modified(1.0, **kwargs)
    if pretrained:
        pretrained_model = model_zoo.load_url(_MODEL_URLS["mnasnet1_0"])
...
if __name__ == '__main__':
    net = mnasnet1_0_modified(pretrained=True)
    print('END', flush=1)
